...
namespace ControlExtensions	// namespace MyWinCollection
{
    public class VirtualizingWrapPanel : VirtualizingPanel, IScrollInfo
    {
        ...
        private void ComputeExtentAndViewport(Size pixelMeasuredViewportSize, int visibleSections)
        {
            ...
            if (Orientation == Orientation.Horizontal)
            {
                _extent.Height = (_abstractPanel?.SectionCount ?? 0) + ViewportHeight - 1;

            }
            else
            {
                _extent.Width = (_abstractPanel?.SectionCount ?? 0) + ViewportWidth - 1;
            }
        ...
        private int GetNextSectionClosestIndex(int itemIndex)
        {
            ...
            if (abstractItem?.Section < _abstractPanel?.SectionCount - 1)
            {
                var ret = _abstractPanel?.
                    ...
                    OrderBy(x => Math.Abs(x.SectionIndex - (abstractItem?.SectionIndex ?? 0))).
                    First();
                return ret._index;
            }
            else
